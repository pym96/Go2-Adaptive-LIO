 ...
 * coordinateRotation(CoordinateAxis:X, 0.1) * v will rotate v by -0.1 radians -
 ...
 * 两个四元数的乘机， robotics modelling planning and control 55页
 */
template <typename T>
...
Quat<typename T::Scalar> integrateQuat(const Eigen::MatrixBase<T>& quat,
  ...
  if (ang > 0) { 
    axis = omega / ang; //单位化，变化率的
  } else {
    axis = Vec3<typename T::Scalar>(1, 0, 0); //如果计算的角度是小于0的，那么直接指定
  }
//变化率
  ang *= dt;
