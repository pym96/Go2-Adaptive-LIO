...
namespace unitree_legged_control
{
    ...
    UnitreeJointController::UnitreeJointController(){
      ...
      // todo use for joint command init
      isupdate = false;
    }
    ...
    // todo 自行实现扭矩计算
    double UnitreeJointController::calTorque(double current_position, double current_velocity, ServoCmd& cmd){
    ...
    void UnitreeJointController::update(const ros::Time& time, const ros::Duration& period)
    {
      ...
      calcTorque = calTorque(currentPos, currentVel, servoCmd);
